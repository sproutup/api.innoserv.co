...
exports.read = function (req, res) {
  var _item;
  Contributor.queryOne('campaignId').eq(req.params.campaignId).where('userId').eq(req.params.userId).attributes(['address', 'bid', 'campaignId', 'comment', 'created', 'id', 'phone', 'state', 'userId', 'trial', 'log']).exec().then(function(item){
...
exports.update = function (req, res) {
  ...
  Contributor.queryOne('campaignId').eq(req.params.campaignId).where('userId').eq(req.params.userId).attributes(['address', 'bid', 'campaignId', 'comment', 'created', 'id', 'phone', 'state', 'userId', 'trial', 'log']).exec()
    .then(function(item){
      if(_.isUndefined(item)){
        return res.status(400).send({
          message: 'Contributor not found'
...
exports.listByUser = function (req, res) {
  ...
  var campaigns = null;
  Contributor.query({userId: req.params.userId}).attributes(['address', 'bid', 'campaignId', 'comment', 'created', 'id', 'phone', 'state', 'userId', 'trial', 'log']).exec().then(function(items){
    contributions = items;
    if(items.length > 0){
